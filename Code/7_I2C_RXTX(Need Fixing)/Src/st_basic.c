...
/*
Function Name: I2C1_Slave_Init
Input Variables: None
Output Variables: None
Desc: Enables and configures I2C1 as the slave
*/
void I2C1_Slave_Init(void)
{
	//Enable clock access to GPIOB
	//Set PB8 mode to alternate function and enable pull-up
	GPIO_Init(GPIOB, 8, GPIO_ALTERNATIVE | GPIO_INPUT_PULLUP);
	//Set PB9 mode to alternate function and enable pull-up
	GPIO_Init(GPIOB, 9, GPIO_ALTERNATIVE | GPIO_INPUT_PULLUP);
	//Set alternate function type to I2C1(AF4)
	GPIOB->AFR[1] |= 4 << GPIO_AFRH_AFSEL8_Pos;
	GPIOB->AFR[1] &= ~(11 << GPIO_AFRH_AFSEL8_Pos);
	GPIOB->AFR[1] |= 4 << GPIO_AFRH_AFSEL9_Pos;
	GPIOB->AFR[1] &= ~(11 << GPIO_AFRH_AFSEL9_Pos);
...
/*
Function Name: I2C1_SetOwnAddress1
Input Variables: input address, address size(0 = 7bit, 1 = 10bit)
Output Variables: None
Desc: Enables and configures I2C1 own address to input address.
*/
void I2C1_SetOwnAddress1(int address, int addressSize)
{
	int newAddress = address | I2C_OAR1_OA1_Msk;
	//Clear OA1 address
	I2C1->OAR1 &= ~(I2C_OAR1_OA1_Msk);
	//Set new OA1 address
	I2C1->OAR1 |= newAddress;
	//Check addressSize
	if(addressSize == 0)
	{
		//Clear OA1MODE bit
...
/*
Function Name: I2C3_Master_Init
Input Variables: None
Output Variables: None
Desc: Enables and configures I2C3 as the Master
*/
void I2C3_Master_Init(void)
{
	//Enable clock access to GPIOC
	//Set PC0 mode to alternate function and enable pull-up
	GPIO_Init(GPIOC, 0, GPIO_ALTERNATIVE | GPIO_INPUT_PULLUP);
	//Set PC1 mode to alternate function and enable pull-up
	GPIO_Init(GPIOC, 1, GPIO_ALTERNATIVE | GPIO_INPUT_PULLUP);
	//Set alternate function type to I2C1(AF4)
	GPIOB->AFR[0] |= 4 << GPIO_AFRL_AFSEL0_Pos;
	GPIOB->AFR[0] &= ~(11 << GPIO_AFRL_AFSEL0_Pos);
	GPIOB->AFR[0] |= 4 << GPIO_AFRL_AFSEL1_Pos;
	GPIOB->AFR[0] &= ~(11 << GPIO_AFRL_AFSEL1_Pos);
	//Set pin as open-drain
...
{
	uint8_t checkNum = (I2C1->ISR | I2C_ISR_ADDCODE_Msk) >> I2C_ISR_ADDCODE_Pos << 1;
	if(compNum == checkNum)
	{
		return 1;
	}
